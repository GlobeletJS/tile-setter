...
uniform float width, miterLimit;
...
mat3 miterTransform(vec2 xHat, vec2 yHat, vec2 v) {
  ...
  float tx = (miterLength > miterLimit)
    ? 0.5 * width * bevelLength
    : 0.5 * width * miterLength;

  float ty = isCap ? 1.2 * width : 0.0;
...
void main() {
  ...
  vec2 extend = miterLimit * xBasis * width * (position.x - 0.5);
  vec2 point = pointB.xy + xAxis * position.x + yBasis * width * position.y + extend;
